...
6.  **Timeline 状态同步**: 好友关系与分组变更均通过全局 Sequence (+ Timeline) 机制保证多端一致性。

## 💡 技术亮点

- **多端同步 (Multi-Device Consistency)**: 采用全局自增序列号 (Sequence) 结合 Timeline 模型，确保好友申请、关系建立、黑名单及分组变更在多个终端（手机、PC、Web）间实时、准确同步。
- **事务一致性 (Atomic Operations)**: 核心业务逻辑（如审批好友、添加成员）均受 `@Transactional` 事务保护，保证数据库更新、序列号递增与消息通知的原子性。
- **高性能查询优化**: 针对高频社交操作（好友校验、申请批处理）进行了 SQL 合并与 Stream 流优化，降低数据库负载。
- **严谨的权限与隐私校验**: 实现了完善的自添加拦截、双向黑名单校验及 `appId` 隔离机制，确保社交数据的安全性。
...
> [!IMPORTANT]
> 本项目需要 **JDK 21** 环境。请确保 `java -version` 输出为 21.x。

确保本地已安装并启动以下中间件：
- **MySQL** (数据库 `im_core`)
- **Redis** (默认端口 6379)
- **Zookeeper** (默认端口 2181，推荐版本 3.8.4)
- **RabbitMQ** (默认端口 5672，使用默认 `guest/guest` 账户)
...
- RabbitMQ 用户 (默认: `guest/guest`)
- Zookeeper 地址 (默认: `localhost:2181`)
