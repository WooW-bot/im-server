...
public class MqMessageProducer {
   ...
   * 发送消息到MQ（通用对象）
   * <p>
   * 根据命令类型自动路由到对应的服务队列： - 命令以1开头 → MessageService - 命令以2开头 → GroupService - 命令以3开头 →
   * FriendshipService - 命令以4开头 → UserService
   *
   ...
   * 参数校验
   */
  private static boolean validateParameters(Object messagePack, MessageHeader header,
      Integer command) {
    if (messagePack == null) {
   ...
   */
  private static void doSendMessage(Object messagePack, MessageHeader header, Integer command,
      String queueName) {
   ...
   * @return JSON消息体
   */
  private static JSONObject buildMessageBody(Object messagePack, MessageHeader header,
      Integer command) {
    // 工业级防御：如果是未解析的原始字节流（说明是未知命令），记录简短警告并包装
    if (messagePack instanceof byte[]) {
      log.warn("Forwarding raw byte[] for unknown command: command={}", command);
    }

    JSONObject messageBody = JsonUtils.toJSONObject(messagePack);
